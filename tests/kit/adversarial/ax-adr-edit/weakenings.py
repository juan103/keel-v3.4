"""ax-adr-edit: Control-1 weakening helpers for the F1 differential-observability gate.
...
Provides the three check_fns the F1 truth table + admissibility controls need:
  ...
  surgical_weak  -- named_check_weak.run: identical to intact EXCEPT the class-predicate
                    anchor _immutable_body_changed is neutralized (returns False); the
                    _consequences_changed clause is UNTOUCHED.
  global_disable -- always-False (a NEGATIVE control: admissibility must REJECT it because
                    it breaks the Consequences-caught (non-predicate) retained witness).
...
_intact_mod = _load("ax_adr_edit_named_check_intact", INTACT_SOURCE)
_weak_mod = _load("ax_adr_edit_named_check_weak", WEAK_SOURCE)
...
def global_disable(tree) -> bool:
    ...
    Not a surgical weakening: it removes EVERY clause (class-predicate AND
    _consequences_changed), so it breaks the Consequences-caught (non-predicate)
    retained witness.  Control-1 admissibility must REJECT it.
    """
    return False
