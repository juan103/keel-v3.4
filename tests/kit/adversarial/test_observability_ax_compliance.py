"""F1 -- the differential-observability gate for ax-compliance (Phase 3.5b).

A thin binding of the ax-compliance scenario pieces to the reusable F1 harness:
...
Scenario observable (principled narrowing -- see scenario_spec.OBSERVABLE_SCOPE):
  the hz-compliance-gameable escape is exercised as a GOALS.md locked-section
  divergence (## Falsifier / ## Commitments softened so it diverges from the
  committed goals_sections snapshot).  The named check WRAPS the REAL
...
_AX_DIR = Path(__file__).resolve().parent / "ax-compliance"
...
_scn_mod = _load("ax_compliance_scenario_spec", _AX_DIR / "scenario_spec.py")
SCENARIO = _scn_mod.AX_COMPLIANCE

_weak = _load("ax_compliance_weakenings", _AX_DIR / "weakenings.py")
INTACT = _weak.intact
...
CLASS_VARIANT = _load(
    "ax_compliance_variant_a", _AX_DIR / "escape_variants" / "variant_a.py"
).mutator

_ANCHOR = "_falsifier_section_diverged"


def test_F1_ax_compliance(tmp_path):
    """F1 (4-cell table + 3 controls) for ax-compliance via the reusable harness."""
