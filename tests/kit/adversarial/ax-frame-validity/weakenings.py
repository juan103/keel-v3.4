"""ax-frame-validity: Control-1 weakening helpers for the F1 gate.
...
Provides the three check_fns the F1 truth table + admissibility controls need:
  ...
  surgical_weak  -- named_check_weak.run: identical to intact EXCEPT the class-predicate
                    anchor _author_verdict_null is neutralized (returns False); the
                    _audit_artifact_missing clause is UNTOUCHED.
  global_disable -- always-False (a NEGATIVE control: admissibility must REJECT it
                    because it breaks the missing-artifact retained witness, a
                    non-predicate witness).
...
_intact_mod = _load("ax_frame_validity_named_check_intact", INTACT_SOURCE)
_weak_mod = _load("ax_frame_validity_named_check_weak", WEAK_SOURCE)
...
def global_disable(tree) -> bool:
    ...
    Not a surgical weakening: it removes EVERY clause (both _author_verdict_null
    AND _audit_artifact_missing), so it breaks the missing-artifact retained
    witness (witness_2_caught_other). Control-1 admissibility must REJECT it.
    """
    return False
