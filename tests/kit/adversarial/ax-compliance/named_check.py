"""ax-compliance: named check (class invariant) for hz-compliance-gameable.
...
CLASS PREDICATE ANCHOR: _falsifier_section_diverged(snapshot_sections, current_sections)
  The class-discriminating clause: the ## Falsifier locked section diverged.

Other clause: _commitments_section_diverged(snapshot_sections, current_sections)
  A SEPARATE top-level clause: the ## Commitments locked section diverged.
...
def _falsifier_section_diverged(snapshot_sections: dict, current_sections: dict) -> bool:
    ...
    True iff the ## Falsifier locked section's current hash diverges from the
    committed snapshot.  This is the class-discriminating clause for
    hz-compliance-gameable: the canonical escape softens the Falsifier section so
    it no longer matches the snapshot, with no adjudicated override.
    """
