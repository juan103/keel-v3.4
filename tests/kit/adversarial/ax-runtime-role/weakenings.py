"""ax-runtime-role: Control-1 weakening helpers for the F1 differential-observability gate.
...
Provides the three check_fns the F1 truth table + admissibility controls need:
  ...
  surgical_weak  -- named_check_weak.run: identical to intact EXCEPT the class-predicate
                    anchor _author_field_unattested is neutralized (returns False); the
                    forgery clause _author_field_token_forged is UNTOUCHED.
  global_disable -- always-False (a NEGATIVE control: admissibility must REJECT it because
                    it breaks the forgery-caught retained witness, a non-predicate witness).
...
_intact_mod = _load("ax_runtime_role_named_check_intact", INTACT_SOURCE)
_weak_mod = _load("ax_runtime_role_named_check_weak", WEAK_SOURCE)
...
def global_disable(tree) -> bool:
    ...
    Not a surgical weakening: it removes EVERY clause (predicate AND forgery), so
    it breaks the forgery-caught (non-predicate) retained witness.  Control-1
    admissibility must REJECT it.
    """
    return False
