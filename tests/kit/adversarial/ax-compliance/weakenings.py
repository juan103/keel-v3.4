"""ax-compliance: Control-1 weakening helpers for the F1 differential-observability gate.
...
Provides the three check_fns the F1 truth table + admissibility controls need:
  ...
  surgical_weak  -- named_check_weak.run: identical to intact EXCEPT the class-predicate
                    anchor _falsifier_section_diverged is neutralized (returns False); the
                    _commitments_section_diverged clause is UNTOUCHED.
  global_disable -- always-False (a NEGATIVE control: admissibility must REJECT it because
                    it breaks the Commitments-caught retained witness, a non-predicate witness).
...
_intact_mod = _load("ax_compliance_named_check_intact", INTACT_SOURCE)
_weak_mod = _load("ax_compliance_named_check_weak", WEAK_SOURCE)
...
def global_disable(tree) -> bool:
    ...
    Not a surgical weakening: it removes EVERY clause (Falsifier AND Commitments),
    so it breaks the Commitments-caught (non-predicate) retained witness.
